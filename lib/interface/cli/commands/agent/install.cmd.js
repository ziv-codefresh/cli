...
const installAgentCmd = new Command({
    ...
    handler: async (argv) => {
        ...
        if (!kubeNamespace) {
            throw new Error('kube-namesapce is a required parameter');
        }
        ...
        await sdk.agent.reportEvent({ agentId: agent.id }, {
            event: {
                type: 'AgentInstalled', kubeNamespace, kubeContextName, exitCode: agentInstallStatusCode,
            },
        });
